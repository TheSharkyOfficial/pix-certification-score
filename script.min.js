...
// Crédit à Yaya.Cout pour la découverte de cette méthode. 😁
...
try {
  ...
  window.alert("Tu dois être connecté sur Pix.")
  throw new Error("Pas connecté - token non trouvé")
}

if (localStorage.getItem("ember_simple_auth-session") == "{\"authenticated\":{}}") {
  window.alert("Tu dois être connecté sur Pix.")
  throw new Error("Pas connecté - token non trouvé")
}
...
function reqListener() {
  ...
  // Traitement des données reçues
	...
	// Détection de la page "Mes certifications" et remplissage direct
    ...
    if (notpublished[0].attributes["pix-score"] == 0) {
      window.alert(`Tu n'as malheureusement pas obtenu ta dernière certification. (${centre})`)
    } else {
      window.alert(`Tu as obtenu ta dernière certification avec ${notpublished[0].attributes["pix-score"]} points ! Félicitations ! (${centre})`)
    }
  }
}
